...
class Index extends Component
{
    ...
    public function store()
    {
        $this->validate([
            'id_sekolah' => 'required',
            'surat_pengantar_sd' => 'required|mimes:jpg,jpeg,png,pdf|max:2048',
            'pkh_kip' => 'required|mimes:jpg,jpeg,png,pdf|max:2048',
        ], [
            'id_sekolah.required' => 'Sekolah harus diisi',
            'surat_pengantar_sd.required' => 'File surat pengantar harus diisi',
            'pkh_kip.required' => 'File PKH/KIP harus diisi',
        ]);
        ...
        if (count($this->dynamicMapping) != $total) {
            ...
            // $data_afirmasi->surat_pengantar_sd = $this->surat_pengantar_sd;
            ...
            $data_afirmasi->surat_pengantar_sd = $fileNameSdmi;


            $files = $this->pkh_kip;
            $fileNameKIPPKH =  'afirmasi-pkh_kip-' . Auth()->user()->id . '.' . $files->getClientOriginalExtension();
            // ORIGINAL
            $destinationPathKIPPKH = public_path('/storage/data_pendaftaran/');
            $img = Image::make($files->getRealPath());
            $QuploadImage = $img->resize(1080, 1080, function ($constraint) {
                $constraint->aspectRatio();
            })->save($destinationPathKIPPKH . $fileNameKIPPKH, 100);
            $data_afirmasi->pkh_kip = $fileNameKIPPKH;
